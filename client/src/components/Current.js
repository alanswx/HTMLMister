...
this.loadCurrentInfo();
	  ...
	  render() {
		  ...
		  const configList = config ? Object.keys(config).map((val,index,arr) => {

			var config_line = config[index][index];
                        var cell=<Table.Cell>{config_line}</Table.Cell>;
			if (index===0) {
                                 Code=config_line;
                                 cell=<Table.Cell><b>{config_line}</b></Table.Cell>;
		  	...
		  	switch(config_line[0])
			  {
				  ...
				  case 'F':
					 var parts=config_line.split(",");
                                         ...
                                         return (<Table.Row><Table.Cell>{"File Load"}</Table.Cell><Table.Cell><Button as={Link} name='Rom List' to={url}>{parts[1]}</Button></Table.Cell></Table.Row>);
				  break;
				  case 'O':
					 var parts=config_line.split(",");
					 ...
					 var drop_title=parts[1];
                                         return (
                                         <Table.Row><Table.Cell>{"Option: "+drop_title}</Table.Cell><Table.Cell>
						 <Dropdown placeholder={drop_title} fluid labeled selection options={dropdown}/>
		...
		<Menu>
		<Menu.Item as={ Link } name='Core List' to='/cores/'>Core List</Menu.Item>
		</Menu>
               <Table celled >
        	 <Table.Header>
                  <Table.Row> 
